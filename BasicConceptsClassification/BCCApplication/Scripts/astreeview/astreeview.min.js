...
if(typeof(gtjs)=="undefined")
_gt=gtjs={};_gt.AjaxHelper=function(){}
...
if(typeof(gtjs)=="undefined")
_gt=gtjs={};_gt.JsonHelper=function(){}
_gt.JsonHelper.isFloat=function(mixed_var){return parseFloat(mixed_var*1)!=parseInt(mixed_var*1,10);}
_gt.JsonHelper.getType=function(mixed_var){var s=typeof mixed_var,name;var getFuncName=function(fn){var name=(/\W*function\s+([\w\$]+)\s*\(/).exec(fn);if(!name){return'(Anonymous)';}
...
else if(s==='number'){s=_gt.JsonHelper.isFloat(mixed_var)?'double':'integer';}
return s;}
_gt.JsonHelper.toArrayJSONString=function(objArray){var a=['['],b,i,l=objArray.length,v;function p(s){if(b){a.push(',');}
a.push(s);b=true;}
for(i=0;i<l;i+=1){v=objArray[i];switch(typeof v){case'undefined':case'function':case'unknown':break;case'object':p(_gt.JsonHelper.__toJSONStringByType(v));break;default:p(_gt.JsonHelper.__toJSONStringByType(v));}}
a.push(']');return a.join('');};_gt.JsonHelper.toBooleanJSONString=function(objBoolean){return String(objBoolean);};_gt.JsonHelper.toDateJSONString=function(objDate){function f(n){return n<10?'0'+n:n;}
return'"'+objDate.getFullYear()+'-'+
f(objDate.getMonth()+1)+'-'+
f(objDate.getDate())+'T'+
f(objDate.getHours())+':'+
f(objDate.getMinutes())+':'+
f(objDate.getSeconds())+'"';};_gt.JsonHelper.toNumberJSONString=function(objNumber){return isFinite(objNumber)?String(objNumber):"null";};_gt.JsonHelper.toObjectJSONString=function(objObject){var a=['{'],b,i,v;function p(s){if(b){a.push(',');}
a.push(_gt.JsonHelper.__toJSONStringByType(i),':',s);b=true;}
for(i in objObject){if(objObject.hasOwnProperty(i)){v=objObject[i];switch(typeof v){case'undefined':case'function':case'unknown':break;case'object':p(_gt.JsonHelper.__toJSONStringByType(v));break;default:p(_gt.JsonHelper.__toJSONStringByType(v));}}}
a.push('}');return a.join('');};_gt.JsonHelper.__toJSONStringByType=function(v){switch(_gt.JsonHelper.getType(v)){case"boolean":return _gt.JsonHelper.toBooleanJSONString(v);case"date":return _gt.JsonHelper.toDateJSONString(v);case"string":return _gt.JsonHelper.toStringJSONString(v);case"integer":case"double":return _gt.JsonHelper.toNumberJSONString(v);case"object":return _gt.JsonHelper.toObjectJSONString(v);case"array":return _gt.JsonHelper.toArrayJSONString(v);default:return"null";}};_gt.JsonHelper.toStringJSONString=function(objString){var m={'\b':'\\b','\t':'\\t','\n':'\\n','\f':'\\f','\r':'\\r','"':'\\"','\\':'\\\\'};if(/["\\\x00-\x1f]/.test(objString)){return'"'+objString.replace(/([\x00-\x1f\\"])/g,function(a,b){var c=m[b];if(c){return c;}
c=b.charCodeAt();return'\\u00'+
Math.floor(c/16).toString(16)+
(c%16).toString(16);})+'"';}
return'"'+objString+'"';};_gt.JsonHelper.parseJSON=function(objStr,hook){try{if(/^("(\\.|[^"\\\n\r])*?"|[,:{}\[\]0-9.\-+Eaeflnr-u \n\r\t])+?$/.test(objStr)){var j=eval('('+objStr+')');if(typeof hook==='function'){function walk(v){if(v&&typeof v==='object'){for(var i in v){if(v.hasOwnProperty(i)){v[i]=walk(v[i]);}}}
return hook(v);}
return walk(j);}
return j;}}catch(e){}
throw new SyntaxError("parseJSON");};if(typeof(gtjs)=="undefined")
_gt=gtjs={};_gt.$=function(id){return"string"==typeof id?document.getElementById(id):id;};_gt.a1=function(className,tag,elm){var testClass=new RegExp("(^|\\s)"+className+"(\\s|$)");var tag=tag||"*";var elm=elm||document;var elements=(tag=="*"&&elm.all)?elm.all:elm.getElementsByTagName(tag);var returnElements=[];var current;var length=elements.length;for(var i=0;i<length;i++){current=elements[i];if(testClass.test(current.className)){returnElements.push(current);}}
return returnElements;}
_gt.a2=function(){this.obj=(arguments.length)?arguments[0]:window;return this;}
_gt.a2.prototype.setInterval=function(func,msec){var i=_gt.a2.getNew();var t=_gt.a2.buildCall(this.obj,i,arguments);_gt.a2.set[i].timer=window.setInterval(t,msec);return i;}
_gt.a2.prototype.setTimeout=function(func,msec){var i=_gt.a2.getNew();_gt.a2.buildCall(this.obj,i,arguments);_gt.a2.set[i].timer=window.setTimeout("_gt.a2.callOnce("+i+");",msec);return i;}
_gt.a2.prototype.clearInterval=function(i){if(!_gt.a2.set[i])return;window.clearInterval(_gt.a2.set[i].timer);_gt.a2.set[i]=null;}
_gt.a2.prototype.clearTimeout=function(i){if(!_gt.a2.set[i])return;window.clearTimeout(_gt.a2.set[i].timer);_gt.a2.set[i]=null;}
_gt.a2.set=new Array();_gt.a2.buildCall=function(obj,i,args){var t="";_gt.a2.set[i]=new Array();if(obj!=window){_gt.a2.set[i].obj=obj;t="_gt.a2.set["+i+"].obj.";}
t+=args[0]+"(";if(args.length>2){_gt.a2.set[i][0]=args[2];t+="_gt.a2.set["+i+"][0]";for(var j=1;(j+2)<args.length;j++){_gt.a2.set[i][j]=args[j+2];t+=", _gt.a2.set["+i+"]["+j+"]";}}
t+=");";_gt.a2.set[i].call=t;return t;}
_gt.a2.callOnce=function(i){if(!_gt.a2.set[i])return;eval(_gt.a2.set[i].call);_gt.a2.set[i]=null;}
_gt.a2.getNew=function(){var i=0;while(_gt.a2.set[i])i++;return i;}
_gt.a3=function(){}
_gt.a3.Bind=function(object,fun){return function(){return fun.apply(object,arguments);}}
_gt.a3.a25=function(object,fun){return function(event){return fun.call(object,(event||window.event));}}
_gt.a3.addEvent=function(oTarget,sEventType,fnHandler){if(oTarget.addEventListener){oTarget.addEventListener(sEventType,fnHandler,false);}else if(oTarget.attachEvent){oTarget.attachEvent("on"+sEventType,fnHandler);}else{oTarget["on"+sEventType]=fnHandler;}};_gt.a3.removeEvent=function(oTarget,sEventType,fnHandler){if(oTarget.removeEventListener){oTarget.removeEventListener(sEventType,fnHandler,false);}else if(oTarget.detachEvent){oTarget.detachEvent("on"+sEventType,fnHandler);}else{oTarget["on"+sEventType]=null;}};_gt.a3.cancelEvent=function(e){e=e||window.event;if(e.preventDefault){e.preventDefault();e.stopPropagation();}else{e.keyCode=0;e.returnValue=false;e.cancelBubble=true;}};_gt.a3.getEventTarget=function(e){e=e||window.event;var obj=e.srcElement?e.srcElement:e.target;return obj};_gt.a3.prev=function(elem){do{elem=elem.previousSibling;}while(elem&&elem.nodeType!=1);return elem;}
_gt.a3.next=function(elem){do{elem=elem.nextSibling;}while(elem&&elem.nodeType!=1);return elem;}
_gt.a3.first=function(elem){elem=elem.firstChild;return elem&&elem.nodeType!=1?_gt.a3.next(elem):elem;}
_gt.a3.last=function(elem){elem=elem.lastChild;return elem&&elem.nodeType!=1?_gt.a3.prev(elem):elem;}
_gt.a3.parent=function(elem,num){num=num||1;for(var i=0;i<num;i++)
if(elem!=null)
elem=elem.parentNode;return elem;}
_gt.a3.trim=function(str){return str.replace(/^\s+|\s+$/g,"");}
_gt.a3.ltrim=function(str){return str.replace(/^\s+/,"");}
_gt.a3.rtrim=function(str){return str.replace(/\s+$/,"");}
_gt.a3.purge=function(d){var a=d.attributes,i,l,n;if(a){l=a.length;for(i=0;i<l;i+=1){n=a[i].name;if(typeof d[n]==='function'){d[n]=null;}}}
a=d.childNodes;if(a){l=a.length;for(i=0;i<l;i+=1){purge(d.childNodes[i]);}}}
_gt.a3.f1=function(el){if(el.getAttribute&&el.getAttribute("is-astreeview-node"))
return el;var p=el.parentNode;while(p){if(p.getAttribute&&p.getAttribute("is-astreeview-node"))
return p;p=p.parentNode;}
return null;}
_gt.a3.f3=function(str){return str.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');}
_gt.a3.f2=function(str){return str.replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&amp;/g,'&');}
_gt.a3.a4=(document.all)?true:false;_gt.a3.a46=_gt.a3.a4&&([/MSIE (\d)+\.0/i.exec(navigator.userAgent)][0][1]==6);_gt.a3.a48=_gt.a3.a4&&([/MSIE (\d)+\.0/i.exec(navigator.userAgent)][0][1]==8);_gt.a3.contains=function(arr,obj){var i=arr.length;while(i--){if(arr[i]===obj){return true;}}
return false;}
_gt.a3.getElementWidth=function(obj)
...
_gt.ASTreeViewNode=function(){this.n1="";this.n2="";this.n3=0;this.n4=0;this.n5=false;this.n6=true;this.n7=true;this.n8=true;this.n9=0;this.n10="";this.n11=true;this.n12=true;this.n13=true;this.n14=true;this.n23=true;this.n24=true;this.n15="";this.n16=false;this.n17=0;this.n18="";this.n19=-1;}
_gt.ASTreeViewLinkNode=function(){this.n20="";this.n21="";this.n22="";}
_gt.ASTreeViewTextNode=function(){}
_gt.ASTreeViewLinkNode.prototype=new _gt.ASTreeViewNode;_gt.ASTreeView=function(insName)
{this.f4=_gt.a3.a25(this,this.f7);this.__d33=_gt.a3.a25(this,this._d33);this._d35=_gt.a3.a25(this,this.d35);this.f5=_gt.a3.a25(this,this.f8);this._d42=_gt.a3.a25(this,this.d42);this._d41=_gt.a3.a25(this,this.d41);this._d46=_gt.a3.a25(this,this.d46);this._d63=_gt.a3.a25(this,this.d63);this._a20=_gt.a3.a25(this,this.a20);this.__d52=_gt.a3.a25(this,this._d52);this.__a22=_gt.a3.a25(this,this._a22);this.__a23=_gt.a3.a25(this,this._a23);this._a24=_gt.a3.a25(this,this.a24);this.__c19=_gt.a3.a25(this,this._c19);this.__d50=_gt.a3.a25(this,this._d50);this.__d51=_gt.a3.a25(this,this._d51);this.__c20=_gt.a3.a25(this,this._c20);this.__bindNodesComplete=_gt.a3.a25(this,this._bindNodesComplete);this.f6=_gt.a3.a25(this,this.f9);this.timer=new _gt.a2(this);this.b2='';this.b3='astreeview-folder.gif';this.b4='astreeview-folder-open.gif';this.b5='astreeview-node.gif';this.b6='astreeview-plus.gif';this.b7='astreeview-minus.gif';this.b8=6;this.b14="astreeview-checkbox-unchecked.gif";this.b15="astreeview-checkbox-checked.gif";this.b16="astreeview-checkbox-half-checked.gif";this.f25="astreeview-dragDrop-indicator1.gif";this.dragDripIndicatorSub="astreeview-dragDrop-indicator2.gif";if(!document.getElementById("b18")){this.b18=document.createElement('UL');this.b18.style.position='absolute';this.b18.style.display='none';this.b18.id='b18';this.b18.className="drag-container";document.body.appendChild(this.b18);}
else
...
((secure)?";secure":"");document.cookie=cookieString;},c4:function(obj){var curleft=curtop=0;if(obj.offsetParent){do{curleft+=obj.offsetLeft;if(obj.tagName=="HTML"||obj.tagName=="BODY")
curtop+=(obj.offsetTop);else
curtop+=(obj.offsetTop-obj.scrollTop);}while(obj=obj.offsetParent);}
...
{var menuItems=document.getElementById(this.b1).getElementsByTagName('LI');for(var i=0;i<menuItems.length;i++){var subItems=menuItems[i].getElementsByTagName('UL');if(subItems.length>0&&menuItems[i].getAttribute("openState")==_gt.ASTreeView.Consts.c11111){this._d33(false,menuItems[i].id);}}},d32:function()
{var menuItems=document.getElementById(this.b1).getElementsByTagName('LI');for(var i=0;i<menuItems.length;i++){var subItems=menuItems[i].getElementsByTagName('UL');if(subItems.length>0&&menuItems[i].getAttribute("openState")==_gt.ASTreeView.Consts.c1111){this._d33(false,menuItems[i].id);}}},f45:function()
{var menuItems=document.getElementById(this.b1).getElementsByTagName('LI');for(var i=0;i<menuItems.length;i++){var subItems=menuItems[i].getElementsByTagName('UL');if(subItems.length>0){this._d33(false,menuItems[i].id);}}},bindNodes:function(parentKey){var container=document.getElementById(this.b1);var ajaxIndex=this.b13.length;this.b13[ajaxIndex]=new _gt.AjaxHelper.ajax();this.b13[ajaxIndex].method="GET";this.b13[ajaxIndex].setVar("virtualParentKey",parentKey);this.__d62(this.b13[ajaxIndex],this.c16);this.b13[ajaxIndex].requestFile=this.c23;var ao=this.b13[ajaxIndex];var fError=this.f6;var fbindNodesComplete=this.__bindNodesComplete;ao.ajaxTrigger=container;(function(){ao.onCompletion=function(){fbindNodesComplete(ao);};})();(function(){ao.onError=function(){fError(ajaxIndex,container);};})();this._showAjaxIndicatorContainer();this.b13[ajaxIndex].runAJAX();},_bindNodesComplete:function(ajaxObject){this._hideAjaxIndicatorContainer();var respRaw=String(ajaxObject.response);var resp="";if(this.f36){var regex=new RegExp(this.f38,"gi");if(regex.test(respRaw))
resp=respRaw.match(this.f38)[0];else
resp=respRaw;}
else
resp=respRaw;if(ajaxObject.ajaxTrigger){var ulPlaceHolder=ajaxObject.ajaxTrigger;ulPlaceHolder.innerHTML=resp;this._initializeNodesUL(ulPlaceHolder);if(this.b41){this.d69(ulPlaceHolder);for(var i=0;i<ulPlaceHolder.childNodes.length;i++){if(ulPlaceHolder.childNodes[i].tagName=="LI"){this.d70(ulPlaceHolder.childNodes[i]);}}}
if(this.enableCheckbox&&this.enableThreeStateCheckbox){var state=ajaxObject.ajaxTrigger.getAttribute("checkedState");if(state==_gt.ASTreeView.Consts.c1){this.d64(ajaxObject.ajaxTrigger,true);}
this.manageNodeCheckboxState(ajaxObject.ajaxTrigger);}
ajaxObject.ajaxTrigger=null;this.onNodesBound();}},_d33:function(e,inputId)
{if(inputId){if(!document.getElementById(inputId))return;thisNode=this.getIcon(document.getElementById(inputId),_gt.ASTreeView.Consts.cc13);}else{var evt=e||window.event;thisNode=evt.target||evt.srcElement;if(!thisNode.getAttribute('is-astreeview-node')){targetA=_gt.a3.f1(thisNode);if(targetA)
thisNode=targetA;}
if(thisNode.tagName=='A')
thisNode=this.getIcon(thisNode.parentNode,_gt.ASTreeView.Consts.cc13);}
if(thisNode.style.visibility=='hidden')return;var parentNode=thisNode.parentNode;inputId=parentNode.id;var isNodeOpening=false;if(thisNode.src.indexOf(this.b6)>=0){isNodeOpening=true;thisNode.src=this.b7;var ul=parentNode.getElementsByTagName('UL')[0];ul.style.display='block';parentNode.setAttribute("openState",_gt.ASTreeView.Consts.c1111)
if(this.c22){if(parentNode.getAttribute("is-virtual-node")=="true"){var obj=parentNode;var virtualParentKey=parentNode.getAttribute("virtual-parent-key");var ajaxIndex=this.b13.length;this.b13[ajaxIndex]=new _gt.AjaxHelper.ajax();this.b13[ajaxIndex].method="GET";this.b13[ajaxIndex].setVar("virtualParentKey",virtualParentKey);this.__d62(this.b13[ajaxIndex],this.c16);this.b13[ajaxIndex].requestFile=this.c23;var ao=this.b13[ajaxIndex];var fError=this.f6;var fc20=this.__c20;ao.ajaxTrigger=obj;(function(){ao.onCompletion=function(){fc20(ao);};})();(function(){ao.onError=function(){fError(ajaxIndex,obj);};})();this._showAjaxIndicatorContainer();this.b13[ajaxIndex].runAJAX();}}}else{thisNode.src=this.b6;parentNode.getElementsByTagName('UL')[0].style.display='none';parentNode.setAttribute("openState",_gt.ASTreeView.Consts.c11111)}
if(!this.c21){var folderIcon=this.getIcon(parentNode,_gt.ASTreeView.Consts.cc15);if(folderIcon){if(parentNode.getAttribute("openState")==_gt.ASTreeView.Consts.c11111)
folderIcon.src=this.b3;else if(parentNode.getAttribute("openState")==_gt.ASTreeView.Consts.c1111)
folderIcon.src=this.b4;}}
if(e){if(this.b48)
this.d72();}
this.onNodeOpenedAndClosed(isNodeOpening?_gt.ASTreeView.Consts.c1111:_gt.ASTreeView.Consts.c11111,parentNode);return false;},d34:function(inputId,state){if(!inputId||!_gt.$(inputId))
return;var imgPlus=_gt.$(inputId).getElementsByTagName('IMG')[0];if(imgPlus.style.visibility=='hidden')return;var liNode=imgPlus.parentNode;if(state==_gt.ASTreeView.Consts.c1111){imgPlus.src=this.b7;var ul=liNode.getElementsByTagName('UL')[0];ul.style.display='block';}
else if(state==_gt.ASTreeView.Consts.c11111){imgPlus.src=this.b6;liNode.getElementsByTagName('UL')[0].style.display='none';}},d35:function(e)
...
currentNode=_gt.a3.f1(currentNode);if(!currentNode)
return;}
...
this.dragNodeDestination=false;this.appendToParent=false;this.az2=0;this.d36();_gt.a3.cancelEvent(evt);return false;},geta2Counter:function(){return _gt.a3.a4?25:40;},d36:function()
...
this.dragDropFlag=true;this.b18.style.display='block';this.b18.appendChild(this.dragNodeSource);}},_getNodeUL:function(node){if(!node)
...
curObj=_gt.a3.f1(curObj);if(!curObj)
...
else if(targetDepth!=sourceDepth){if(_gt.a3.a4)
...
eventSourceObj=_gt.a3.f1(eventSourceObj);if(this.dragNodeDisableSiblings&&eventSourceObj.tagName=='IMG')eventSourceObj=eventSourceObj.nextSibling;var tmpImg=tmpObj.getElementsByTagName('IMG')[0];if((!this.f40&&(eventSourceObj.tagName=='A'||this.dragNodeDisableSiblings))||(this.appendToParent&&(eventSourceObj.tagName=='A'||this.dragNodeDisableSiblings))||(eventSourceObj.tagName=='DIV'&&eventSourceObj.getAttribute('is-astreeview-node')=='true')){var enableChildAttr=curObj.getAttribute("enable-children");if(enableChildAttr&&enableChildAttr.toLowerCase()=="false")
{if(_gt.a3.a4)
window.event.cancelBubble=true;else if(e)
e.stopPropagation();return;}
if(tmpImg.src!=this.f24)
tmpImg.src=this.f24;this.b21=true;var rtlDisplayOffset=0;var rtlOffsetX=0;if(this.enableRightToLeftRender){var objWidth=parseInt(_gt.a3.getElementWidth(eventSourceObj));var indicatorWidth=parseInt(_gt.a3.getElementWidth(tmpObj));if(!isNaN(objWidth)&&objWidth>0){rtlDisplayOffset=objWidth;rtlOffsetX=0-(this.indicatorOffsetXSub*2+indicatorWidth);}}
tmpObj.style.left=(this.c6(eventSourceObj)+this.indicatorOffsetXSub+rtlDisplayOffset+rtlOffsetX)+'px';tmpObj.style.top=(this.c5(curObj)+this.indicatorOffsetY)+'px';}else{if(this.f40){if(targetDepth!=sourceDepth){if(_gt.a3.a4)
window.event.cancelBubble=true;else if(e)
e.stopPropagation();this.appendToParent=false;return;}
this.appendToParent=false;}
if(tmpImg.src!=this.f25)
tmpImg.src=this.f25;this.b21=false;var indicatorPos=this.c4(curObj);var rtlDisplayOffset=0;var rtlOffsetX=0;if(this.enableRightToLeftRender){var objWidth=parseInt(_gt.a3.getElementWidth(curObj));var indicatorWidth=parseInt(_gt.a3.getElementWidth(tmpObj));if(!isNaN(objWidth)&&objWidth>0){rtlDisplayOffset=objWidth;rtlOffsetX=0-(this.indicatorOffsetX*2+indicatorWidth);}}
...
if(this.dragNodeDestination){if(this.b21){var returnVal=this.onDragDropCompleting(this.dragNodeSource,this.dragNodeDestination);if(returnVal!=null&&returnVal===false){this.dragNodeDestination=false;}}
...
if(this.dragNodeDestination.getAttribute("enable-open-close")!='false'){var imgPlusMinus=this.getIcon(this.dragNodeDestination,_gt.ASTreeView.Consts.cc13);imgPlusMinus.style.visibility='visible';imgPlusMinus.src=this.b7;}
this.dragNodeDestination.setAttribute("openState",_gt.ASTreeView.Consts.c1111);if(!this.c21){var imgIcon=this.getIcon(this.dragNodeDestination,_gt.ASTreeView.Consts.cc15);if(imgIcon)
...
this.onDragDropCompleted(this.dragNodeSource,this.dragNodeSource.parentNode.parentNode);if(this.b41){this.d69(this.dragNodeParent);this.d69(this.dragNodeSource.parentNode);for(var i=0;i<this.dragNodeSource.childNodes.length;i++){if(this.dragNodeSource.childNodes[i].tagName=="UL"){this.d69(this.dragNodeSource.childNodes[i]);}}
...
var tmpObj=this.dragNodeParent;var lis=tmpObj.getElementsByTagName('LI');if(lis.length==0){var img=this.getIcon(tmpObj.parentNode,_gt.ASTreeView.Consts.cc13);if(img)
img.style.visibility='hidden';if(!this.c21){var lastImgIcon=this.getIcon(tmpObj.parentNode,_gt.ASTreeView.Consts.cc15);if(lastImgIcon)
lastImgIcon.src=this.b5;}
tmpObj.parentNode.removeChild(tmpObj);}}else{if(this.dragNodeSourceNextSib){this.dragNodeParent.insertBefore(this.dragNodeSource,this.dragNodeSourceNextSib);}else{this.dragNodeParent.appendChild(this.dragNodeSource);}}
this.b20.style.display='none';this.az2=-1;if(showMessage&&this.b81)alert(this.b81);if(document.all){currentObj.releaseCapture();}else{_gt.a3.removeEvent(window,"blur",this.f5);};if(this.b48){this.d72();if(relatedTreeToMove)
...
var lis;lis=initObj.getElementsByTagName('LI');if(lis&&lis.length>0){var li=lis[0];while(li){if(li.getAttribute&&li.getAttribute("treeNodeValue")!=null){var nodeInfo;var treeNodeType=parseInt(li.getAttribute("treeNodeType"));if(treeNodeType==_gt.ASTreeView.Consts.cc11)
nodeInfo=new _gt.ASTreeViewNode();else if(treeNodeType==_gt.ASTreeView.Consts.cc12)
nodeInfo=new _gt.ASTreeViewLinkNode();else if(treeNodeType==_gt.ASTreeView.Consts.NODE_TYPE_TEXT_LINK)
nodeInfo=new _gt.ASTreeViewTextNode();else
continue;var isTextNode=treeNodeType==_gt.ASTreeView.Consts.NODE_TYPE_TEXT_LINK;var curA=isTextNode?this.d75(li,"DIV"):this.d75(li,"A");var liValue=li.getAttribute("treeNodeValue");var curCheckedState=li.getAttribute("checkedState");var curOpenState=li.getAttribute("openState");var curSelected=false;if(li.getAttribute("selected")&&li.getAttribute("selected")=="true")
...
curEnableOpenClose=false;nodeInfo.n11=curEnableDragDrop;nodeInfo.n12=curEnableSiblings;nodeInfo.n13=curEnableChildren;nodeInfo.n14=curEnableCheckbox;nodeInfo.n23=curEnableSelection;nodeInfo.n24=curEnableOpenClose;nodeInfo.n9=treeNodeType;if(this.f40){if(li.getAttribute("tree-node-depth"))
...
if(treeNodeType==_gt.ASTreeView.Consts.cc12){nodeInfo.n20=curA.href;nodeInfo.n21=curA.target;nodeInfo.n22=curA.title;}
...
{if(this.d47(obj))return;var img=this.getIcon(obj,_gt.ASTreeView.Consts.cc13);img.style.visibility='hidden';if(!this.c21){var folderIcon=this.getIcon(obj,_gt.ASTreeView.Consts.cc15);if(folderIcon){folderIcon.src=this.b5;}}},_deleteItemStep2:function(obj)
...
{this._hideAjaxIndicatorContainer();var rawResp=ajaxObject.response;if(ajaxObject.response==_gt.ASTreeView.Consts.cc16){this._finishDelete(ajaxObject.ajaxTrigger);return;}
var repTokens=rawResp.split('|');if(repTokens.length>1)
{if(repTokens[0]!=_gt.ASTreeView.Consts.cc16){this._displayDeleteError(ajaxObject.response);}
else{repTokens.shift();this._finishDelete(ajaxObject.ajaxTrigger,rawResp.substring(2));}}
else
this._displayDeleteError(ajaxObject.response);},_displayDeleteError:function(raw){alert('An error occured while deleting node: '+raw);},_finishDelete:function(trigger,info){if(trigger){var valToDel=trigger.getAttribute("treeNodeValue");var returnVal=this.onNodeDeleted(valToDel,info);if(returnVal!=null&&returnVal===false){return;}
...
this.manageNodeCheckboxState(parentRef);trigger=null;}},_showAjaxIndicatorContainer:function(){if(this.f30&&document.getElementById(this.f30))
...
newNodeText=window.prompt(this.c9,this.addNodePromptDefaultValue);if(!newNodeText||_gt.a3.trim(newNodeText)==""||!curNodeValue)
return;if(this.f34)
newNodeText=_gt.a3.f3(newNodeText);this._addItemStep2(newNodeText,curNodeValue,curNode);},_addItemStep2:function(newNodeText,parentNodeValue,obj){var ajaxIndex=this.b13.length;this.b13[ajaxIndex]=new _gt.AjaxHelper.ajax();this.b13[ajaxIndex].method="GET";this.b13[ajaxIndex].setVar("addNodeText",encodeURIComponent(newNodeText));this.b13[ajaxIndex].setVar("parentNodeValue",encodeURIComponent(parentNodeValue));this.__d62(this.b13[ajaxIndex],this.c15);this.b13[ajaxIndex].requestFile=this.c10;var ao=this.b13[ajaxIndex];var fError=this.f6;var fAddComplete=this.__c19;ao.ajaxTrigger=obj;(function(){ao.onCompletion=function(){fAddComplete(ao);};})();(function(){ao.onError=function(){fError(ajaxIndex,obj);};})();this._showAjaxIndicatorContainer();this.b13[ajaxIndex].runAJAX();},_c19:function(ajaxObject)
{this._hideAjaxIndicatorContainer();var respRaw=String(ajaxObject.response);var resp="";if(this.f36){var regex=new RegExp(this.f38,"gi");if(regex.test(respRaw))
resp=respRaw.match(this.f38)[0];else
resp=respRaw;}
else
    resp = respRaw; var liStr = resp.match(/<li[.\s\S(?!<li)]*?<\/li>/i); if (!liStr) { location.reload();}else{if(ajaxObject.ajaxTrigger){var liParent=ajaxObject.ajaxTrigger;var div=document.createElement("div");div.innerHTML=String(liStr);if(div.childNodes.length>0){var returnVal=this.onNodeAdded(div.childNodes[0]);if(returnVal!=null&&returnVal===false)
return;}
var imgPM=this.getIcon(liParent,_gt.ASTreeView.Consts.cc13);if(imgPM){imgPM.style.visibility='visible';imgPM.src=this.b7;}
if(!this.c21){var folderIcon=this.getIcon(liParent,_gt.ASTreeView.Consts.cc15);if(folderIcon){folderIcon.src=this.b4;}}
...
if(this.enableCheckbox){var state=liParent.getAttribute("checkedState");if(state==_gt.ASTreeView.Consts.c1){for(var i=0;i<ulParent.childNodes.length;i++){if(ulParent.childNodes[i].tagName=="LI")
...
inputObj.value=_gt.a3.f3(inputObj.value);if(inputObj.value.length>0){inputObj.nextSibling.innerHTML=inputObj.value;if(this.b466){if(this.b304!=this.b301){return;}
this.b304=this.b303;var ajaxIndex=this.b13.length;this.b13[ajaxIndex]=new _gt.AjaxHelper.ajax();this.b13[ajaxIndex].method="GET";this.b13[ajaxIndex].setVar("nodeValue",encodeURIComponent(inputObj.parentNode.getAttribute("treeNodeValue")));this.b13[ajaxIndex].setVar("newNodeText",encodeURIComponent(inputObj.value));this.__d62(this.b13[ajaxIndex],this.b84);this.b13[ajaxIndex].requestFile=this.c11;this.b13[ajaxIndex].onError=function(){this.f6(ajaxIndex);};var ao=this.b13[ajaxIndex];ao.ajaxTrigger=inputObj.parentNode;var fEditComplete=this.__d51;(function(){ao.onCompletion=function(){fEditComplete(ao);};})();var fError=this.f6;(function(){ao.onError=function(){fError(ajaxIndex);};})();this._showAjaxIndicatorContainer();this.b13[ajaxIndex].runAJAX();}
else{if(this.b48)
this.d72();var returnVal=this.onNodeEdited(inputObj.parentNode);if(returnVal!=null&&returnVal===false){var aTag=this.d75(inputObj.parentNode,"A");aTag.innerHTML=this.b30.innerHTML;}}}},_d51:function(ajaxObject)
{this._hideAjaxIndicatorContainer();var rawResp=ajaxObject.response;if(ajaxObject.response==_gt.ASTreeView.Consts.cc16){this._finishEdit(ajaxObject.ajaxTrigger);return;}
var repTokens=rawResp.split('|');if(repTokens.length>1)
{if(repTokens[0]!=_gt.ASTreeView.Consts.cc16){this._displayEditError(ajaxObject.response);}
else{repTokens.shift();this._finishEdit(ajaxObject.ajaxTrigger,rawResp.substring(2));}}
else
this._displayEditError(ajaxObject.response);},_displayEditError:function(raw){alert('An error occured while renaming node: '+raw);},_finishEdit:function(trigger,info){if(trigger){var returnVal=this.onNodeEdited(trigger,info);if(returnVal!=null&&returnVal===false){var aTag=this.d75(trigger,"A");aTag.innerHTML=this.b30.innerHTML;}}},_a23:function(e,inputObj)
...
{if(document.all)e=event;var elem=e.srcElement||e.target;if(e.keyCode==13&&!this.f32){this._d52(false,elem);_gt.a3.cancelEvent(e);}
if(e.keyCode==27){this._a23(false,elem);}},_d55:function(obj)
{var textBox=this.f32?document.createElement('TEXTAREA'):document.createElement('INPUT');textBox.className='astreeview-edit-box';textBox.value=this.f34?_gt.a3.f2(obj.innerHTML):obj.innerHTML;obj.parentNode.insertBefore(textBox,obj);textBox.id='textBox'+obj.parentNode.getAttribute("treeNodeValue");_gt.a3.addEvent(textBox,"blur",this.__d52);_gt.a3.addEvent(textBox,"keydown",this.__a22);this._d56(obj);},_d56:function(obj)
{this.b304=this.b301;obj.style.display='none';obj.previousSibling.value=this.f34?_gt.a3.f2(obj.innerHTML):obj.innerHTML;obj.previousSibling.style.display='inline';obj.previousSibling.focus();obj.previousSibling.select();},editItem:function(obj1,obj2)
...
{this.d39();_gt.a3.addEvent(document.documentElement,"selectstart",this._d42);_gt.a3.addEvent(document.documentElement,"dragstart",this.d41);_gt.a3.addEvent(document.documentElement,"mousedown",this._d46);this.b30=document.createElement('DIV');this.b30.style.display='none';document.body.appendChild(this.b30);if(this.b27||this.b26){try{}catch(e){}}
var astObj=document.getElementById(this.b1);this._initializeNodesUL(astObj);_gt.a3.addEvent(document.documentElement,"mousemove",this.f4);_gt.a3.addEvent(document.documentElement,"mouseup",this.f5);},_initializeNodesUL:function(ulObj){var menuItems=ulObj.getElementsByTagName('LI');for(var i=0;i<menuItems.length;i++){this._d57Node(menuItems[i]);}},_d57Node:function(liNode){if(liNode.getAttribute("virtial-node-placeholder-li")=="true")
return;var nodeEnableDragDrop=true;var tmpVar=liNode.getAttribute('enable-drag-drop');if(tmpVar&&tmpVar=='false')
nodeEnableDragDrop=false;var subItems=liNode.getElementsByTagName('UL');var imgPlusMinus=this.getIcon(liNode,_gt.ASTreeView.Consts.cc13);_gt.a3.addEvent(imgPlusMinus,"click",this.__d33);if(subItems.length>0)
{subItems[0].id='t_u_'+liNode.getAttribute("treeNodeValue");this.b49++;}
var aTag=this.d75(liNode,"A");if(aTag&&nodeEnableDragDrop&&this.enableDragDrop){_gt.a3.addEvent(aTag,"mousedown",this._d35);_gt.a3.addEvent(aTag,"mousemove",this.f4);if(this.f44){_gt.a3.addEvent(aTag.parentNode,"mousedown",this._d35);_gt.a3.addEvent(aTag.parentNode,"mousemove",this.f4);}}
if(this.enableCheckbox){var cbCheck=this.getIcon(liNode,_gt.ASTreeView.Consts.cc14);_gt.a3.addEvent(cbCheck,"click",this._d63);}
var folderImg=this.getIcon(liNode,_gt.ASTreeView.Consts.cc15);if(folderImg){if(nodeEnableDragDrop&&this.enableDragDropOnIcon&&this.enableDragDrop){_gt.a3.addEvent(folderImg,"mousedown",this._d35);_gt.a3.addEvent(folderImg,"mousemove",this.f4);}}
var disableCurrentNodeSelection=(liNode.getAttribute("enable-selection")=="false");if(aTag&&this.b34&&!disableCurrentNodeSelection){_gt.a3.addEvent(aTag,"click",this._a20);}},__d58:function(curNode){if(curNode)
...
state=parseInt(state);if((state==_gt.ASTreeView.Consts.c1)||(state==_gt.ASTreeView.Consts.c11&&includeHalfChecked)){var val=li.getAttribute("treeNodeValue");result+=(val+splitter);}}
...
elm=this.getIcon(elm,_gt.ASTreeView.Consts.cc14);var state=nodeItem.getAttribute("checkedState");if(state!=_gt.ASTreeView.Consts.c1||forceCheck){nodeItem.setAttribute("checkedState",_gt.ASTreeView.Consts.c1);elm.src=this.b15;if(this.enableThreeStateCheckbox)
this.d65(nodeItem,_gt.ASTreeView.Consts.c1);}
else{nodeItem.setAttribute("checkedState",_gt.ASTreeView.Consts.c111);elm.src=this.b14;if(this.enableThreeStateCheckbox)
this.d65(nodeItem,_gt.ASTreeView.Consts.c111);}},d65:function(targetNode,state){var childNodes=this.__d58(targetNode);for(var i=0;i<childNodes.length;i++){var curNode=childNodes[i];if(curNode.getAttribute("virtial-node-placeholder-li")=="true")
continue;var curCheckbox=this.getIcon(curNode,_gt.ASTreeView.Consts.cc14);curNode.setAttribute("checkedState",state);if(state==_gt.ASTreeView.Consts.c1)
curCheckbox.src=this.b15;else if(state==_gt.ASTreeView.Consts.c111)
curCheckbox.src=this.b14;else if(state==_gt.ASTreeView.Consts.c11)
curCheckbox.src=this.b16;}},manageNodeCheckboxState:function(node){while(node){if(node.id&&node.id==this.b1)
break;var children=this.__d59(node);var totalChildrenCount=children.length;var checkedNodesCount=0;var halfCheckedNodesCount=0;for(var i=0;i<children.length;i++){var curChildNode=children[i];if(curChildNode.getAttribute("checkedState")==_gt.ASTreeView.Consts.c1)
checkedNodesCount++;if(curChildNode.getAttribute("checkedState")==_gt.ASTreeView.Consts.c11)
halfCheckedNodesCount++;}
var curCheckbox=_gt.a1("astreeview-checkbox","IMG",node)[0];if(halfCheckedNodesCount>0)
{node.setAttribute("checkedState",_gt.ASTreeView.Consts.c11);curCheckbox.src=this.b16;}
else{if(checkedNodesCount==0){node.setAttribute("checkedState",_gt.ASTreeView.Consts.c111);curCheckbox.src=this.b14;}
else if(checkedNodesCount==totalChildrenCount){node.setAttribute("checkedState",_gt.ASTreeView.Consts.c1);curCheckbox.src=this.b15;}
else{node.setAttribute("checkedState",_gt.ASTreeView.Consts.c11);curCheckbox.src=this.b16;}}
...
else{var totalChildrenCount=children.length;var checkedNodesCount=0;var halfCheckedNodesCount=0;for(var i=0;i<children.length;i++){var curChildNode=children[i];var curState=this.d68(curChildNode);if(curState==_gt.ASTreeView.Consts.c1)
checkedNodesCount++;if(curChildNode.getAttribute("checkedState")==_gt.ASTreeView.Consts.c1&&this.__d59(curChildNode).length>0)
checkedNodesCount++;if(curChildNode.getAttribute("checkedState")==_gt.ASTreeView.Consts.c11&&this.__d59(curChildNode).length>0)
halfCheckedNodesCount++;}
var curCheckbox=_gt.a1("astreeview-checkbox","IMG",node)[0];if(halfCheckedNodesCount>0)
{node.setAttribute("checkedState",_gt.ASTreeView.Consts.c11);curCheckbox.src=this.b16;}
else{if(checkedNodesCount==0){node.setAttribute("checkedState",_gt.ASTreeView.Consts.c111);curCheckbox.src=this.b14;}
else if(checkedNodesCount==totalChildrenCount){node.setAttribute("checkedState",_gt.ASTreeView.Consts.c1);curCheckbox.src=this.b15;}
else{node.setAttribute("checkedState",_gt.ASTreeView.Consts.c11);curCheckbox.src=this.b16;}}}},clearTreeLines:function(obj){var cn=obj.className;cn=cn.replace(this.b44,"");cn=cn.replace(this.c17,"");cn=cn.replace(this.c18,"");cn=cn.replace(this.b43,"");cn=cn.replace(this.b42,"");cn=cn.replace(this.b45,"");obj.className=cn;},isSingleRootNode:function(node){return(node.parentNode.childNodes.length==1)&&node.parentNode==document.getElementById(this.b1);},isTopRootNode:function(node){return(node.parentNode.childNodes.length>1&&node.parentNode==document.getElementById(this.b1)&&node==node.parentNode.firstChild);},d70:function(node){var parentUL=_gt.a3.parent(node);if(!parentUL)
return;if(_gt.a3.last(parentUL)==node){this.clearTreeLines(node);var str=node.className;str+=(" "+this.b44);node.className=str;}
else{this.clearTreeLines(node);var str=node.className;str+=(" "+this.b43);node.className=str;}
var childUL=_gt.a3.last(node);if(childUL&&childUL.tagName=="UL")
...
if(_gt.a3.last(grandpa)==parentUL.parentNode){this.clearTreeLines(parentUL);this.clearTreeLines(parentUL.parentNode);var str=parentUL.className;str+=(" "+this.b45);parentUL.className=str;str=parentUL.parentNode.className;str+=(" "+this.b44);parentUL.parentNode.className=str;return;}}
...
elm=_gt.a3.f1(elm);var node=elm.parentNode;if(!node)
return;if(this.f27&&this.__d59(node).length>0){var imgPlusMinus=this.getIcon(node,_gt.ASTreeView.Consts.cc13);this.__d33(e,node.id);}
...
lastA.className=lastA.className.replace(this.b33,"");}},d72:function(){if(this.b37!=""&&_gt.$(this.b37))
_gt.$(this.b37).value=_gt.JsonHelper.toArrayJSONString(this.d43());},d73:function(){return this.b1;},d74:function(id){var result=false;for(var i=0;i<this.b47.length;i++){if(eval(this.b47[i]+".d73()")==id){return this.b47[i];break;}}
